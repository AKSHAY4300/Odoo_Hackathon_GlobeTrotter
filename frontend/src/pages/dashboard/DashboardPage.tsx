...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="bg-ink-navy text-runway-white rounded-3xl p-6 sm:p-8 border border-ink-navy-700 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="bg-boarding-amber text-ink-navy font-mono text-[11px] font-bold px-2.5 py-0.5 rounded uppercase">
                TRAVEL CONCIERGE TERMINAL
              </span>
              <span className="text-xs font-mono text-tarmac-grey-300">
                PASSPORT HOLDER: {user?.name || 'Explorer'}
              </span>
            </div>

            <h1 className="font-display font-bold text-2xl sm:text-4xl text-white tracking-tight">
              {getGreeting()}, {user?.name?.split(' ')[0] || 'Traveler'}! ✈️
            </h1>

            <p className="text-xs sm:text-sm text-tarmac-grey-200 max-w-xl leading-relaxed">
              Your personalized multi-city expedition hub. You currently have{' '}
              <strong className="text-boarding-amber font-mono font-bold">{trips.length} curated itineraries</strong>{' '}
              spanning <strong className="text-signal-teal font-mono font-bold">{totalDestinationsVisited} destination stops</strong>.
            </p>
          </div>
        ...
        {/* 2. Highlights Summary Strip */}
        <div className="mt-8 pt-6 border-t border-white/10 grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-white/5 p-3.5 rounded-2xl border border-white/10">
            <span className="text-[10px] font-mono uppercase text-tarmac-grey-300 block">Active Voyages</span>
            <span className="text-xl sm:text-2xl font-mono font-bold text-white mt-0.5 block">{trips.length} Trips</span>
          </div>

          <div className="bg-white/5 p-3.5 rounded-2xl border border-white/10">
            <span className="text-[10px] font-mono uppercase text-tarmac-grey-300 block">Destinations Mapped</span>
            <span className="text-xl sm:text-2xl font-mono font-bold text-boarding-amber mt-0.5 block">
              {totalDestinationsVisited} Stops
            </span>
          </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display font-bold text-xl md:text-2xl text-ink-navy flex items-center gap-2">
              <span>Your Voyages & Boarding Passes</span>
            </h2>
            <p className="text-xs text-tarmac-grey mt-0.5">
              Personalized multi-city journeys you have chartered
            </p>
          </div>
        ...
        {tripsLoading ? (
          ...
          <div className="bg-white rounded-3xl border-2 border-dashed border-tarmac-grey/25 p-8 text-center space-y-3 shadow-xs">
            ...
            <p className="text-xs text-tarmac-grey max-w-sm mx-auto">
              Stamp your first passport page by creating an itinerary, choosing departure dates, and adding beautiful city stops.
            </p>
            <Link to="/trips/new">
      ...
      <section className="space-y-4 pt-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-tarmac-grey mt-0.5">
              Hand-curated global hubs with rich activity catalogs and scenic landmarks
            </p>
          </div>
